...
os.chdir("C:\\Users\\qlm573\\melanoma-identification")
metadata_path = os.path.join(os.getcwd(), "images", "metadata")

data = pd.read_csv(os.path.join(metadata_path, "metadata.csv"))
...
isicIDs = data["isic_id"]  # length of 71, 670
print(len(isicIDs), len(isicIDs.unique()), len(data["lesion_id"].unique()))
