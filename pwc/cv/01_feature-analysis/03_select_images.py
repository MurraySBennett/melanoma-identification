...
def main(target_n=100, save_data=False):
    ...
    # )
    shape_data = read_data(os.path.join(here, 'shape.txt'))
    shape_data['id'] = shape_data['id'].apply(lambda i: i.split('.')[0])
    duplicate_ids = get_txt(os.path.join(here, "rm-list.txt"))
    malignant_ids = get_txt(os.path.join(here, "malignant_ids.txt"))
    shape_data = shape_data.merge(malignant_ids, how='left', on='id')
    shape_data = shape_data[~shape_data['id'].isin(duplicate_ids['isic_id'])]
    shape_data["isic_id"] = shape_data["id"]
    ...
    sampled_ids = [i+'.JPG' for i in sampled_ids]

    if save_data:
        np.savetxt(os.path.join(here, "feature-rating-image-list.txt"), sampled_ids, fmt="%s", header="id", comments="")
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(here, "cv_features.pdf"), format="pdf", dpi=600, bbox_inches="tight")
    plt.show()


def read_data(path):
    return pd.read_csv(path, delim_whitespace=True, header=0)
