...
file_handler = logging.FileHandler('shape.txt')
...
def flip_mask_h(mask, section = 'top'):
    ...
    if section == 'top':
        ...
        mirror = cv.flip(mask[midpoint:, :], 0)
        flipped_mask[:midpoint, :] = mirror
    # mirror_top = cv.flip(mask[:midpoint, :], 0)
    # flipped_mask[midpoint:,:] = mirror_top
...
def combine_halves(mask, axis='horz'):
    ...
    if axis == 'horz':
        ...
        # combined[:midpoint, :] = cv.bitwise_or(mask[:midpoint,:], mirror)
    return combined
...
def get_asymmetry(mask, contour):
    ...
    # major axis aligned and centered - no morphological changes
    # Lx = rotated_mask
    # add bottom half to top, mirror combined top to bottom, return difference between 'bottom' added to 'top'
    # dLx = combine_halves(Lx, axis='horz')

    # Ly = rotate_mask(Lx, rotation_angle = 90)
    # dLy = combine_halves(Ly, axis='horz')

    x_symmetry = np.round(np.divide(np.sum(dLx), np.sum(Lx)), 4)
    y_symmetry = np.round(np.divide(np.sum(dLy), np.sum(Ly)), 4)

    # print(f'x: {x_symmetry}, y: {y_symmetry}')
    # show_images([mask, rotated_mask, Lx, dLx, Ly, dLy])
...
def measure_shape(mask):
    ...
    try:
        ...
        # logger.info(f'error with: {label}')
